  ...
  scanCode: function (e) {
    ...
    wx.scanCode({
      ...
      success(res) {
        ...
        const authUri = "https://tinas.fengjijiao.cn/qrc?";
        if (res.result.substring(0, authUri.length) != authUri) {
          wx.showToast({
            title: '没有发现图片上的信息!',
            icon: 'none',
            duration: 3000
          });
          console.log('没有发现图片上的信息!');
        } else {
          try {
            console.log("Qr操作信息返回:", unescape(Base64.decode(res.result.substr(authUri.length))));
            var result = JSON.parse(unescape(Base64.decode(res.result.substr(authUri.length))));
